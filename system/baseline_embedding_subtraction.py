...
def get_sim_subtracted(concept1, concept2, prop, model):
    ...
    vec_sub = subtract(concept1, concept2, model)
...
def get_sim_subtracted_rev(concept1, concept2, prop, model):
...
def extract_features_rev(triples, model):
    ...
    for triple in triples:
        ...
        prop = triple[2]
        if check_vocab([concept1, concept2, prop], model):
            features.append([get_sim_subtracted_rev(concept1, concept2, prop, model)])
        else:
            features.append([0.0])
...
def embedding_subtraction_classification_baseline_rev(data_train, data_test, model, hidden_layer, activation):


    hiddel_layer_str = '-'.join([str(l) for l in hidden_layer])

    name = 'baseline_subtraction_rev_'+data_test
...
if __name__ == '__main__':
    ...
    embedding_subtraction_classification_baseline_rev(data_train, data_test, model, hidden_layer, activation)
